...
static uint
mirror_bmap(struct inode *ip, uint bn, int copy)
{
  uint addr, *a;
  struct buf *bp;
...
stati(struct inode *ip, struct stat *st)
{
  ...
  {
    st->physical_size = (ip->physical_size * 2);
  }
  else
  {
    st->physical_size = ip->physical_size;
  }
...
int
readi(struct inode *ip, char *dst, uint off, uint size)
{
  uint tot, m;
  struct buf *bp, *mirroredbp;

  if(ip->type == T_DEV)
  {
    if(ip->major < 0 || ip->major >= NDEV || !devsw[ip->major].read)
      return -1;
    return devsw[ip->major].read(ip, dst, size);
  }


    //checks for valid offest
    if(off > ip->size || off + size < off)
    {
      return -1;
    }

    //Makes sure you dont read outside of the file
    if(off + size > ip->size)
    {
      size = ip->size - off;
    }
    ...
    for(tot = 0; tot < size; tot += m, off += m, dst += m)
    {
      ...
      if(ip->type == T_MIRRORED)
      {
        ...
        bp = bread(ip->dev, mirror_bmap(ip, off/BSIZE, FIRSTCOPY));
        //m = min(size - tot, BSIZE - off % BSIZE);

        mirroredbp = bread(ip->dev, mirror_bmap(ip, off/BSIZE, SECONDCOPY));
        m = min(size - tot, BSIZE - off % BSIZE);

        //compare the data
        if( memcmp(bp->data, mirroredbp->data, m) < 0 )
        {
          brelse(bp);
          brelse(mirroredbp);          
          return -1;
...
writei(struct inode *ip, char *src, uint off, uint n)
{
  ...
  for(tot = 0; tot < n; tot += m, off += m, src += m)
  {
    if(ip->type == T_MIRRORED)
    {
      ...
      memmove(mirroredbp->data + off%BSIZE, src, m);
